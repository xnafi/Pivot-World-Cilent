...
const CourseDetails = () => {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 h-full md:h-full w-full px-8 bgCourse bg-center bg-fixed bg-cover bg-no-repeat flex flex-col md:flex-row justify-center items-center" >
      ...
      <div className="md:w-3/4 w-screen px-2 md:px-0 rounded-md shadow-md dark:bg-gray-900 dark:text-gray-50 bg-slate-700 my-10 mt-28" ref={ref}>
        ...
        <div className='flex justify-between flex-col md:flex-row items-center px-4'>
          <Pdf className='bg-slate-500' targetRef={ref} filename="code-example.pdf">
            {({ toPdf }) =>
              <button className="btn btn-outline btn-warning my-2" onClick={toPdf}>Download</button>}
          </Pdf>
          <Link to={`/purchase/${id}`} className="btn btn-outline btn-info text-sm my-2">Get premium access</Link>
